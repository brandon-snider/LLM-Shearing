...
class CoFiLayerNorm(torch.nn.LayerNorm):
    ...
    def prune_params(self, hidden_z):
        ...
        self.weight = torch.nn.parameter.Parameter(
            self.weight.index_select(0, remaining_index)
        )
        self.bias = torch.nn.parameter.Parameter(
            self.bias.index_select(0, remaining_index)
        )
...
class PythiaEmbedding(nn.Embedding):
    ...
    def prune_params(self, hidden_z):
        ...
        self.weight = torch.nn.parameter.Parameter(
            self.weight.index_select(1, remaining_index).clone()
        )
        ...
        self.transformer.update(
            {
                "ln_f": layernorm_class(
                    cfg.d_model, eps=cfg.layer_norm_eps, device=cfg.init_device
                ),  # TODO: add to config
            }
        )
...
class PythiaBlock(nn.Module):
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        self.use_parallel_residual = cfg.get(
    ...
    def forward(
        ...
        if self.ln_1 is not None:
            ...
            attn_output, _, past_key_value = self.attn(
                a,
                past_key_value=past_key_value,
                attn_bias=attn_bias,
                key_padding_mask=key_padding_mask,
        ...
        if self.use_parallel_residual:
            ...
            if self.ln_2 is not None:
                ...
                mlp_output = self.mlp(
                    hidden_states, retain_grad, intermediate_z, mlp_z, hidden_z
                )
...
class PythiaAttention(nn.Module):
    ...
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        self.query_key_value = nn.Linear(
            self.d_model, 3 * self.d_model, device=device, bias=True
        )
        fuse_splits = (cfg.d_model, 2 * cfg.d_model)
        ...
        self.rotary_emb = RotaryEmbedding(
            self.rotary_ndims, max_position_embeddings=cfg.max_seq_len, device=device
        )
    ...
    def prune_params(self, zs_block):
        ...
        if hidden_z is not None:
            ...
            self.query_key_value = prune_linear_layer(
                self.query_key_value, remaining_index, dim=1
            )
    ...
    def forward(
        ...
        position_ids = torch.arange(
            offset, kv_seq_len, dtype=torch.long, device=cos.device
        )
...
class PythiaMLP(nn.Module):
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        self.down_proj = nn.Linear(
            cfg.intermediate_size, cfg.d_model, bias=True, device=device
        )
        self.up_proj = nn.Linear(
            cfg.d_model, cfg.intermediate_size, bias=True, device=device
        )
    ...
    def prune_params(self, zs_block):
        ...
        if intermediate_z is not None:
            self.down_proj.weight.data = self.down_proj.weight.data.mul(
                intermediate_z.squeeze(0)
            )
