...
def generate_work(
    ...
    assert (
        epoch == 0
    ), "Currently only supports dynamic loading from each domain for once."
    # Ensure that num_canonical_nodes has been set.
    if dataset.num_canonical_nodes is None:
        raise RuntimeError(
            f"`num_canonical_nodes` can never be None. "
    ...
    for stream_id, stream in enumerate(dataset.streams):
        ...
        if dataset.shuffle:
            ...
            used_stream_ids = np.array(
                [
                    reverse_mapping[used_stream_ids[j]]
                    for j in range(len(used_stream_ids))
                ],
                dtype=np.int64,
            )
            del reverse_mapping
        ...
        # check
        stream_partition = get_partitions_orig(
            samples_in_stream,
            dataset.num_canonical_nodes,
            world.num_nodes,
        ...
        if dataset.shuffle:
            ...
            stream_partition = np.where(
                stream_partition != -1, stream_shuffle[stream_partition], -1
            )
        ...
        assert (
            np.intersect1d(
                partition_per_stream[-1].flatten(), np.array(used_domain_ids[stream_id])
            ).size
            == 0
        )
...
class DynamicStreamingDataset(StreamingDataset):
    ...
    def __init__(
        ...
        *,
        ...
        streams = [
            Stream(local=local, split=set_name, repeat=1.0) for set_name in set_names
        ]
        super().__init__(
            streams=streams,
            split=None,
    ...
    def state_dict(
        self, used_sample_ids: List[List[int]], from_beginning: bool
    ) -> Dict[str, Any]:
        ...
        domain_sample_in_epoch = [
            len(used_sample_ids[i]) for i in range(len(self.set_names))
        ]
    ...
    def _resume(self, world: World, epoch: int) -> Tuple[int, int]:
        ...
        try:
            ...
            return epoch, np.array(
                [[] for _ in range(self.num_streams)], dtype=np.int64
            )
    ...
    def _share_work(
        self, sample_ids_per_stream: List[List[int]]
    ) -> List[Tuple[SharedMemory, SharedMemory]]:
        ...
        for stream_id, sample_ids in enumerate(sample_ids_per_stream):
            ...
            if sample_ids.ndim != ndim:
                raise ValueError(
                    f"Sample IDs must be of {ndim}D shape (num physical nodes, "
            ...
            shape_shm = SharedMemory(
                name=name, create=True, size=size, auto_cleanup=False
            )
    ...
    def _attach_work(self) -> Tuple[NDArray[np.int64], SharedMemory, SharedMemory]:
        ...
        for stream_id in range(self.num_streams):
            ...
            size = ndim * np.int64().nbytes
            shape_shm = SharedMemory(
                name=name, create=False, size=size, auto_cleanup=False
            )
    ...
    def _get_work(
        self, world: World, epoch: int, used_domain_ids: List[List[int]]
    ) -> NDArray[np.int64]:
        ...
        sample_ids_per_stream = [
            sample_ids_per_stream[stream_id][
                world.node, world.rank_of_node, world.worker_of_rank
            ].flatten()
            for stream_id in range(self.num_streams)
        ]
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        while True:
            proportion = self.proportion
            stream_id = np.random.choice(range(self.num_streams), 1, p=proportion)[
                0
            ].item()
            domain_sample_id = sample_ids_per_stream[stream_id]
            domain_sample_id = domain_sample_id[
                self.used_num_samples_per_stream[stream_id]
...
class TextDynamicStreamingDataset(DynamicStreamingDataset):
    ...
    def _read_binary_tokenized_sample(self, sample):
        if self.is_uint16:
            a = (
                np.frombuffer(sample["tokens"], dtype="B")
