...
def merge_splits_with_no_bias():
    ...
    if args.input_dir is None:
        args.input_dir = [
            Path(os.path.dirname(args.split_names[i]))
            for i in range(len(args.split_names))
        ]
        args.split_names = [
            os.path.basename(args.split_names[i]) for i in range(len(args.split_names))
        ]
    else:  # if input dirs are different, then only pass split_names in
        args.input_dir = [Path(args.input_dir)] * len(args.split_names)

    first_index_file = json.load(
        open(args.input_dir[0] / args.split_names[0] / "index.json", "r")
    )
    ...
    # shuffle
    if args.shuffle:
        for i, (input_dir, split_name) in enumerate(
            zip(args.input_dir, args.split_names)
        ):
