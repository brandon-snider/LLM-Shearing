...
for split in folders:
    split_path = os.path.join(args.tokenized_dir, split, "**/*.npy")
    files.extend(
        os.path.join(split, os.path.basename(f))
        for f in glob.glob(split_path, recursive=True)
    )
...
for folder in target_folders:
    ...
    # Use the first quarter of files as evaluation
    selected = folder_to_files[folder][: len(folder_to_files[folder]) // 4]

    # Use the remaining files for training
    folder_to_files[folder] = folder_to_files[folder][
        len(folder_to_files[folder]) // 4 :
    ]
